...
if [[ -s "${PROJECT_DIR}"/.github_token ]]; then
	...
	if [[ "${GIT_ORG}" == "${GIT_USER}" ]]; then
		...
		MESSAGE="Firmware Dumped Successfully, Pushing ${repo} to GitHub in The Organization ${GIT_ORG} Privately"
		live_telegram_update
	else
		...
		MESSAGE="Firmware Dumped Successfully, Pushing ${repo} to GitHub Privately"
		live_telegram_update
	fi
